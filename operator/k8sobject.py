...
class K8sObject:
    ...
    async def create(cls, definition):
        ...
        if namespace is None:
            return cls(
                await Poolboy.custom_objects_api.create_cluster_custom_object(
                    group = cls.api_group,
                    plural = cls.plural,
    ...
    async def delete(self):
        try:
            if self.namespace is None:
                self.definition = await Poolboy.custom_objects_api.delete_cluster_custom_object(
                ...
                self.definition = await Poolboy.custom_objects_api.delete_namespaced_custom_object(
                    group = self.api_group,
                    name = self.name,
    ...
    async def merge_patch(self, patch):
        if self.namespace is None:
            self.definition = await Poolboy.custom_objects_api.patch_cluster_custom_object(
                group = self.api_group,
                name = self.name,
                plural = self.plural,
                version = self.api_version,
            ...
            self.definition = await Poolboy.custom_objects_api.patch_namespaced_custom_object(
                group = self.api_group,
                name = self.name,
                namespace = self.namespace,
                plural = self.plural,
    ...
    async def merge_patch_status(self, patch):
        if self.namespace is None:
            self.definition = await Poolboy.custom_objects_api.patch_cluster_custom_object_status(
                group = self.api_group,
                name = self.name,
                plural = self.plural,
                version = self.api_version,
            ...
            self.definition = await Poolboy.custom_objects_api.patch_namespaced_custom_object_status(
                group = self.api_group,
                name = self.name,
                namespace = self.namespace,
                plural = self.plural,
    ...
    async def refetch(self):
        self.definition = cls.fetch_definition(self.name, self.namespace)
