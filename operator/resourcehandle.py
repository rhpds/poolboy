...
class ResourceHandleMatch:
    ...
    def __lt__(self, cmp):
        ...
        if self.resource_handle.is_ready and cmp.resource_handle.is_ready is False:
            return True
        if self.resource_handle.is_ready is False and cmp.resource_handle.is_ready:
            return False

        return (
            self.resource_handle.creation_timestamp
            < cmp.resource_handle.creation_timestamp
        )
...
class ResourceHandle(KopfObject):
    ...
    async def bind_handle_to_claim(
        ...
        async with cls.class_lock:
            ...
            for name in Cache.get_keys_by_tag(CacheTag.HANDLE_UNBOUND):
                ...
                if (
                    resource_claim.resource_pool_name
                    and resource_claim.resource_pool_name
                    != resource_handle.resource_pool_name
                ):
                    continue

                # Do not bind to handles that are near end of lifespan
                if (
                    resource_handle.has_lifespan_end
                ...
                match.resource_count_difference = len(resource_claim_resources) - len(
                    handle_resources
                )
                ...
                for i, handle_resource in enumerate(handle_resources):
                    ...
                    provider = await resourceprovider.ResourceProvider.get(
                        provider_name
                    )
            ...
            for match in matches:
                ...
                for resource_index, claim_resource in enumerate(
                    ...
                    if resource_index < len(matched_resource_handle.resources):
                        handle_resource = matched_resource_handle.resources[
                            resource_index
                        ]
                        ...
                        patch_value = {
                            "provider": resource_claim_resources[resource_index][
                                "provider"
                            ],
                        }
                        if resource_name:
                ...
                lifespan_default = matched_resource_handle.get_lifespan_default(
                    resource_claim
                )
                if lifespan_default:
                    patch.append(
                        {
                            ...
                            "value": (
                                ...
                                + matched_resource_handle.get_lifespan_default_timedelta(
                                    resource_claim
                                )
                            ).strftime("%FT%TZ"),
                        }
                ...
                try:
                    ...
                    if exception.status == 404:
                        logger.warning(
                            f"Attempt to bind deleted {matched_resource_handle} to {resource_claim}"
                        )
        ...
        if matched_resource_handle.is_from_resource_pool:
            resource_pool = await resourcepool.ResourcePool.get(
                matched_resource_handle.resource_pool_name
            )
            if resource_pool:
    ...
    async def create_for_claim(
        ...
        if resource_claim.has_resource_provider:
            ...
            lifespan_default_timedelta = (
                resource_provider.get_lifespan_default_timedelta(resource_claim)
            )
            lifespan_maximum = resource_provider.lifespan_maximum
            lifespan_maximum_timedelta = (
                resource_provider.get_lifespan_maximum_timedelta(resource_claim)
            )
            lifespan_relative_maximum = resource_provider.lifespan_relative_maximum
            lifespan_relative_maximum_timedelta = (
                resource_provider.get_lifespan_maximum_timedelta(resource_claim)
            )
        else:
            ...
            for i, claim_resource in enumerate(resource_claim_resources):
                ...
                provider_lifespan_default_timedelta = (
                    provider.get_lifespan_default_timedelta(resource_claim)
                )
                if provider_lifespan_default_timedelta:
                    if (
                        not lifespan_default_timedelta
                        or provider_lifespan_default_timedelta
                        < lifespan_default_timedelta
                    ):
                        lifespan_default_timedelta = provider_lifespan_default_timedelta

                provider_lifespan_maximum_timedelta = (
                    provider.get_lifespan_maximum_timedelta(resource_claim)
                )
                if provider_lifespan_maximum_timedelta:
                    if (
                        not lifespan_maximum_timedelta
                        or provider_lifespan_maximum_timedelta
                        < lifespan_maximum_timedelta
                    ):
                        lifespan_maximum = provider.lifespan_maximum
                        lifespan_maximum_timedelta = provider_lifespan_maximum_timedelta

                provider_lifespan_relative_maximum_timedelta = (
                    provider.get_lifespan_relative_maximum_timedelta(resource_claim)
                )
                if provider_lifespan_relative_maximum_timedelta:
                    if (
                        not lifespan_relative_maximum_timedelta
                        or provider_lifespan_relative_maximum_timedelta
                        < lifespan_relative_maximum_timedelta
                    ):
        ...
        if requested_lifespan_end_datetime:
            ...
            lifespan_end_datetime = (
                lifespan_start_datetime + lifespan_relative_maximum_timedelta
            )
        ...
        if lifespan_end_datetime:
            ...
            if (
                lifespan_maximum_timedelta
                and lifespan_end_datetime
                > lifespan_start_datetime + lifespan_maximum_timedelta
            ):
                logger.warning(
                    f"Requested lifespan end {resource_claim.requested_lifespan_end_timestamp} "
                    f"for ResourceClaim {resource_claim.name} in {resource_claim.namespace} "
        ...
        if lifespan_default_timedelta:
            definition["spec"].setdefault("lifespan", {})["default"] = timedelta_to_str(
                lifespan_default_timedelta
            )

        if lifespan_end_datetime:
            definition["spec"].setdefault("lifespan", {})["end"] = (
                lifespan_end_datetime.strftime("%FT%TZ")
            )
        ...
        if lifespan_relative_maximum:
            definition["spec"].setdefault("lifespan", {})["relativeMaximum"] = (
                lifespan_relative_maximum
            )
    ...
    async def create_for_pool(
        ...
        if resource_pool.has_resource_provider:
            ...
            if resource_provider.has_lifespan:
                ...
                if resource_provider.lifespan_default:
                    definition["spec"]["lifespan"]["default"] = (
                        resource_provider.lifespan_default
                    )
                if resource_provider.lifespan_maximum:
                    definition["spec"]["lifespan"]["maximum"] = (
                        resource_provider.lifespan_maximum
                    )
                if resource_provider.lifespan_relative_maximum:
                    definition["spec"]["lifespan"]["maximum"] = (
                        resource_provider.lifespan_relative_maximum
                    )
                if resource_provider.lifespan_unclaimed:
                    definition["spec"]["lifespan"]["end"] = (
                        datetime.now(timezone.utc)
                        + resource_provider.lifespan_unclaimed_timedelta
                    ).strftime("%FT%TZ")
        else:
        ...
        if resource_pool.has_lifespan:
            ...
            if resource_pool.lifespan_default:
                definition["spec"]["lifespan"]["default"] = (
                    resource_pool.lifespan_default
                )
            if resource_pool.lifespan_maximum:
                definition["spec"]["lifespan"]["maximum"] = (
                    resource_pool.lifespan_maximum
                )
            if resource_pool.lifespan_relative_maximum:
                definition["spec"]["lifespan"]["relativeMaximum"] = (
                    resource_pool.lifespan_relative_maximum
                )
            if resource_pool.lifespan_unclaimed:
                definition["spec"]["lifespan"]["end"] = (
                    datetime.now(timezone.utc)
                    + resource_pool.lifespan_unclaimed_timedelta
                ).strftime("%FT%TZ")
        ...
        if Poolboy.is_standalone:
            resource_handle.__register()
        logger.info(
            f"Created ResourceHandle {resource_handle.name} for ResourcePool {resource_pool.name}"
        )
        return resource_handle
    ...
    async def delete_unbound_handles_for_pool(
        ...
        """Delete all unbound handles for a pool."""
    ...
    @classmethod
    async def get(
        cls, name: str, ignore_deleting=True, use_cache=True
    ) -> ResourceHandleT | None:
        async with cls.class_lock:
            if use_cache:
            ...
            async with cls.class_lock:
                for name in Cache.get_keys_by_tag(CacheTag.HANDLE_UNBOUND):
                    ...
                    if (
                        ...
                        and resource_handle.resource_pool_namespace
                        == resource_pool.namespace
                    ):
        ...
        while True:
            ...
            for definition in resource_handle_list["items"]:
                ...
                if not resource_handle.is_bound:
                    ...
                    resource_handle.cache_set(
                        CacheTag.HANDLE, resource_handle.name, ttl=300
                    )
                    resource_handle.cache_set(
                        CacheTag.HANDLE_UNBOUND, resource_handle.name, ttl=300
                    )
                    resource_handles.append(resource_handle)
        ...
        async with cls.class_lock:
            ...
            while True:
                resource_handle_list = (
                    await Poolboy.custom_objects_api.list_namespaced_custom_object(
                        group=Poolboy.operator_domain,
                        namespace=Poolboy.namespace,
                        plural="resourcehandles",
                        version=Poolboy.operator_version,
    ...
    def is_healthy(self) -> bool | None:
        ...
        for resource in self.status_resources:
            if resource.get("healthy") is False:
                return False
            if (
                resource.get("waitingFor") is not None
                and resource.get("healthy") is None
            ):
                ret = None
        return ret
    ...
    def is_ready(self) -> bool | None:
        ...
        for resource in self.status_resources:
            if resource.get("ready") is False:
                return False
            if resource.get("waitingFor") is not None and resource.get("ready") is None:
                ret = None
        return ret
    ...
    def __lifespan_value_as_timedelta(
        self,
        name: str,
        resource_claim: ResourceClaimT,
    ) -> timedelta | None:
        ...
        if seconds is None:
            raise kopf.TemporaryError(
                f"Failed to parse {name} time interval: {value}", delay=60
            )
        return timedelta(seconds=seconds)

    async def __manage_init_status_resources(
        self,
        logger: kopf.ObjectLogger,
    ) -> None:
    ...
    async def __manage_check_delete(
        self, logger: kopf.ObjectLogger, resource_claim: ResourceClaimT
    ) -> bool:
        ...
        if self.is_past_lifespan_end:
            logger.info(
                f"Deleting {self} at end of lifespan ({self.lifespan_end_timestamp})"
            )
            await self.delete()
            return True
        if self.is_bound and not resource_claim:
            logger.info(
                f"Propagating deletion of {self.resource_claim_description} to {self}"
            )
            await self.delete()
            return True

    async def __manage_update_spec_resources(
        self,
        logger: kopf.ObjectLogger,
    ...
    def get_lifespan_default_timedelta(self, resource_claim=None):
        return self.__lifespan_value_as_timedelta(
            "default", resource_claim=resource_claim
        )
    ...
    def get_lifespan_maximum_timedelta(self, resource_claim=None):
        return self.__lifespan_value_as_timedelta(
            "maximum", resource_claim=resource_claim
        )
    ...
    def get_lifespan_relative_maximum_timedelta(self, resource_claim=None):
        return self.__lifespan_value_as_timedelta(
            "relativeMaximum", resource_claim=resource_claim
        )

    def get_lifespan_end_maximum_datetime(self, resource_claim=None):
        lifespan_start_datetime = (
            resource_claim.lifespan_start_datetime
            if resource_claim
            else self.creation_datetime
        )

        maximum_timedelta = self.get_lifespan_maximum_timedelta(
            resource_claim=resource_claim
        )
        if maximum_timedelta:
            if resource_claim.lifespan_first_ready_timestamp:
                maximum_end = (
                    resource_claim.lifespan_first_ready_datetime + maximum_timedelta
                )
            else:
        ...
        relative_maximum_timedelta = self.get_lifespan_relative_maximum_timedelta(
            resource_claim=resource_claim
        )
        if relative_maximum_timedelta:
            relative_maximum_end = (
                datetime.now(timezone.utc) + relative_maximum_timedelta
            )
        else:
            relative_maximum_end = None

        if relative_maximum_end and (
            not maximum_end or relative_maximum_end < maximum_end
        ):
    ...
    async def get_resource_provider(self) -> ResourceProviderT | None:
        ...
        if self.resource_provider_name:
            return await resourceprovider.ResourceProvider.get(
    ...
    async def get_resource_providers(self) -> List[ResourceProviderT]:
        ...
        for resource in self.spec.get("resources", []):
            resource_providers.append(
                await resourceprovider.ResourceProvider.get(
    ...
    async def handle_delete(self, logger: kopf.ObjectLogger) -> None:
            ...
            if reference:
                try:
                    resource_description = (
                        f"{reference['apiVersion']} {reference['kind']} "
                        + (
                            f"{reference['name']} in {reference['namespace']}"
                            if "namespace" in reference
                            else reference["name"]
                        )
                    )
                    logger.info(
                        f"Propagating delete of {self} to {resource_description}"
                    )
        ...
        if resource_claim and not resource_claim.is_detached:
            ...
            logger.info(
                f"Propagated delete of {self} to ResourceClaim {resource_claim}"
            )
    ...
    async def manage(self, logger: kopf.ObjectLogger) -> None:
        ...
        async with self.lock:
            ...
            if await self.__manage_check_delete(
            ...
            for resource_index in range(len(self.resources)):
                ...
                for linked_provider in resource_provider.linked_resource_providers:
                    ...
                    for pn, pv in enumerate(resource_providers):
                        if (
                            pv.name == linked_provider.name
                            and self.resources[pn].get("name", pv.name)
                            == linked_provider.resource_name
                        ):
                ...
                resource_definition = (
                    await resource_provider.resource_definition_from_template(
                        logger=logger,
                        resource_claim=resource_claim,
                        resource_handle=self,
                ...
                resource_namespace = resource_definition["metadata"].get(
                    "namespace", None
                )
                ...
                if "reference" not in status_resource:
                    ...
                    if resource_namespace != status_resource["reference"].get(
                        "namespace"
                    ):
                        resource_namespace = status_resource["reference"]["namespace"]
                        resource_definition["metadata"]["namespace"] = (
                            resource_namespace
                        )

                resource_description = (
                    f"{resource_api_version} {resource_kind} {resource_name}"
                )
                if resource_namespace:
                ...
                if resource_state:
                    ...
                    if updated_state:
                        ...
                        logger.info(
                            f"Updated {resource_description} for ResourceHandle {self.name}"
                        )
                else:
            ...
            for resource_index, resource_definition in resources_to_create:
                ...
                if resource_namespace:
                    resource_description += f" in {resource_namespace}"
                try:
                    created_resource = await poolboy_k8s.create_object(
                        resource_definition
                    )
                    if created_resource:
            ...
            # This is needed in worker context where ResourceWatch runs in the
            # operator process and may not immediately sync with worker changes.
            if Poolboy.is_worker and (
                resources_to_create or resources_updated or patch
            ):
    ...
    async def update_status(
        ...
        while len(self.resources) < len(resource_states):
            logger.warning(
                f"{self} update status with resource states longer that list of resources, attempting refetch: {len(self.resources)} < {len(resource_states)}"
            )
            await asyncio.sleep(0.2)
            await self.refetch()
            if len(self.resources) < len(resource_states):
                logger.error(
                    f"{self} update status with resource states longer that list of resources after refetch: {len(self.resources)} < {len(resource_states)}"
                )
                return
        ...
        for idx, resource in enumerate(resources):
            ...
            if state:
                resource_provider = await resourceprovider.ResourceProvider.get(
                    resource["provider"]["name"]
                )
