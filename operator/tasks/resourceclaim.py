...
def _dispatch_batch(claims: list) -> int:
    ...
    for item in claims:
        ...
        manage_claim.apply_async(kwargs=kwargs, task_id=f"claim-{uid}-{rv}")
        dispatched += 1
    return dispatched
...
async def _manage_claim(definition: dict) -> dict:
    ...
    if not claim:
        ...
        return {"status": "skipped", "reason": "not_found", "claim": definition['metadata']['name']}
...
def maintain_all_claims():
    ...
    with distributed_lock(lock_key, timeout=300) as acquired:
        ...
        logger.info(f"Claim maintenance: {len(claims)} claims in {len(batches)} batches")
