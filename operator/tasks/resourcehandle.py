...
def _dispatch_batch(handles: list) -> int:
    ...
    which is necessary for time-based triggers like lifespan.end.
    """
    import time

    ts_minute = int(time.time() // 60)  # One dispatch allowed per minute per handle
    ...
    for item in handles:
        ...
        manage_handle.apply_async(
            kwargs=kwargs, task_id=f"handle-sched-{uid}-{ts_minute}"
        )
        dispatched += 1
    return dispatched
...
async def _manage_handle(definition: dict) -> dict:
    ...
    if not handle:
        ...
        return {
            "status": "skipped",
            "reason": "not_found",
            "handle": definition["metadata"]["name"],
        }

    # Register handle in cache for binding operations
    # This ensures unbound handles are available for claim binding
    await handle.register_definition(handle.definition)
...
def maintain_all_handles():
    ...
    with distributed_lock(lock_key, timeout=300) as acquired:
        ...
        logger.info(
            f"Handle maintenance: {len(handles)} handles in {len(batches)} batches"
        )
